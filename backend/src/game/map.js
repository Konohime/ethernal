...
class DungeonMap extends DungeonComponent {
  ...
  registerEventHandlers() {
    ...
    // PATCH: Ajout des handlers pour RoomDiscovered et RoomActualised
    events.on(Dungeon, 'RoomDiscovered', this.handleRoomDiscovered.bind(this));
    events.on(Dungeon, 'RoomActualised', this.handleRoomActualised.bind(this));
  ...
  const valid = Object.values(rooms)
    ...
    // .filter(({ coordinates }) => Object.keys(bfs(rooms, coordinates, 0)).length >= 5) // Disabled for dev - need 5+ connected rooms
